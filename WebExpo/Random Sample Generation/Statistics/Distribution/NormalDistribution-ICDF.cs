namespace Zygotine.Statistics.Distribution
{
    ...
    public static partial class NormalDistribution
    {
        public static double QNorm(double p, double mu = 0.0, double sigma = 1.0, bool lower_tail = true, bool log_p = false)
        {
            ...
            if (sigma < 0) return double.NaN; //ML_ERR_return_NAN;
        ...
        public static double[] QNorm(double[] p, double mu = 0.0, double sigma = 1.0, bool lower_tail = true, bool log_p = false)
        {
            return QNorm(p, mu: new double[] { mu }, sigma: new double[] { sigma }, lower_tail: lower_tail, log_p: log_p);
        }

        public static double[] QNorm(double[] p, double[] mu, double[] sigma, bool lower_tail = true, bool log_p = false)
        {

            int n = p.Length > mu.Length ? p.Length : mu.Length;
            if (n < sigma.Length)
                n = sigma.Length;
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                rep[i] = QNorm(p[iP], mu[iMu], sigma[iSigma], lower_tail, log_p);
            }
