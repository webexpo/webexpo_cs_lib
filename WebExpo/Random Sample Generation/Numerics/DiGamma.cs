namespace Zygotine.Numerics
{
    ...
    public class DiGamma
    {
        ...
        /*
         * Error vaudra ierr, sauf lorsque dpsifn n'est pas appellé
         * Il faudra alors -1;
         * 
         */

        public double Value { get; private set; } = Tools.ND; // ans
        public double X { get; private set; } // x
        public int Error { get; private set; } = 0; //ierr
        public int Nz { get; private set; } = 0;

        public DiGamma(double x)
        {
            this.X = x;
            double[] ans = new double[] { double.NaN };
            int nz = 0;
            int ierr = 0;
            if (double.IsNaN(x))
            {
            ...
            dpsifn(x, 0, 1, 1, ans, ref nz, ref ierr);
            this.Error = ierr;
            this.Nz = nz;
            this.Value = (ierr != 0) ? double.NaN : -ans[0];
        }

        internal static double R_pow_di(double x, int n)
        {
            double xn = 1.0;
            ...
            if (n != 0)
            {
                if (!double.IsNaN(x)) return Math.Pow(x, n);
        ...
        private static void dpsifn(double x, int n, int kode, int m, double[] ans, ref int nz, ref int ierr)
        {
            const int n_max = 100;

            double[] bvalues = {	/* Bernoulli Numbers */
            ...
            xm, xmin, xq, yint;
            double[] trm = new double[23], trmr = new double[n_max + 1];
            ...
            if (x <= 0.0)
            {
                ...
                if (x == Math.Round(x))
                {
                    ...
                    for (j = 0; j < m; j++) /* k = j + n : */
                        ans[j] = ((j + n) % 2) != 0 ? double.PositiveInfinity : double.NaN;
                    return;
                }
                /* This could cancel badly */
                dpsifn(1.0 - x, n, /*kode = */ 1, m, ans, ref nz, ref ierr);
                ...
                if (n == 0)
                    ...
                    tt = -1 / R_pow_di(Math.Sin(x), 2);
                else if (n == 2)
                    tt = 2 * Math.Cos(x) / R_pow_di(Math.Sin(x), 3);
                else if (n == 3)
                    tt = -2 * (2 * R_pow_di(Math.Cos(x), 2) + 1.0) / R_pow_di(Math.Sin(x), 4);
                else /* can not happen! */
                    tt = double.NaN;
                /* end cheat */

                s = ((n % 2) != 0) ? -1.0 : 1.0;/* s = (-1)^n */
            ...
            mm = m;
            nx = Math.Min(-Rf_i1mach(15), Rf_i1mach(16));/* = 1021 */
            r1m5 = Rf_d1mach(5);
        ...
        L20:
            for (i = 1; i <= nx; i++)
                s += 1.0 / (x + (nx - i)); /* avoid disastrous cancellation, PR#13714 */
        ...
        * fonction non utilisée
        */
        private static double fmax2(double x, double y)
        {
            //La seule nuance que peut introduire la somme serait peut-être 
            // de pouvoir distinguer NaN et NA.
            // Sinon Math.Min(NaN, 1) == NaN
            //# ifdef IEEE_754
            if (ISNAN(x) || ISNAN(y))
                return x + y;
            //#endif
        ...
        private static double Rf_d1mach(int i)
        {
            //const double DBL_MIN = 2.2250738585072014e-308;
            //const double DBL_MAX = 1.7976931348623158e+308;
            //const double M_LOG10_2 = 0.301029995663981195213738894724;
         ...
         * Penser à éliminer
         */
        private static int Rf_i1mach(int i)
        {
